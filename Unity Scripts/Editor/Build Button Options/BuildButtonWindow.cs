...
public class BuildButtonWindow : EditorWindow
{
    ...
    public static string AndroidBuildPath   => "Builds/" + EditorPrefs.GetString("AndroidBuildPath") + ".apk";
    public static string PcBuildPath        => "Builds/" + EditorPrefs.GetString("PCBuildPath") + ".exe";
    ...
    {
        EditBuildPathsWindow window = EditorWindow.GetWindow<EditBuildPathsWindow>("Edit Build Paths");
        window.Show();
    }
    ...
    private static BuildPlayerOptions CreateAndroidBuildOptions()
    {
        ...
        androidBuildOptions.extraScriptingDefines = new[] { "ENABLE_BURST_DEBUG_INFORMATION=0" };
...
public class EditBuildPathsWindow : EditorWindow
{
    ...
    {
        EditBuildPathsWindow window = (EditBuildPathsWindow)EditorWindow.GetWindow(typeof(EditBuildPathsWindow));
        fullPathPrefix = Application.dataPath.Substring(0, Application.dataPath.LastIndexOf("/"));
    ...
    string RemovePrefix(string path) => path.Substring("/Builds/".Length + fullPathPrefix.Length);
    private void OnGUI()
    {
        ...
        if (showFullPaths){
            ...
            androidBuildPath = androidBuildPath.Substring(0, androidBuildPath.Length - ".apk".Length);
